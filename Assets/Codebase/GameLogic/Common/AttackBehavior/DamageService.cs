...
namespace Assets.Codebase.GameLogic.Common.AttackBehavior
{
    public class DamageService : IDamageService
    {
        ...
        private bool IsTargetFor(IAttacker attacker, Collider2D hit, out IDamageable target)
        {
            if (hit != null && !hit.isTrigger && hit.transform.gameObject != null)
            {
                return hit.transform.TryGetComponent(out target) && hit.transform.gameObject != attacker.GameObject;
            }
